...
describe('urlBuilder', () => {
  describe('compile', () => {
    ...
    it('compiles a single transform, applying and overriding default transformations', () => {
      ...
      expect(compiledTransformVideo).toBe('/w_auto,h_140,c_fill,r_30');
    });
    ...
    it('compiles an array of transforms without applying defaults', () => {
      ...
      expect(compiledTransformVideo).toBe(
        '/w_220,h_140,c_fill' +
          '/l_brown_sheep,w_220,h_140,x_220,c_fill' +
          '/l_horses,w_220,h_140,y_140,x_-110,c_fill' +
          '/w_400,h_260,r_20,c_crop' +
  ...
  describe('urlBuilder', () => {
    ...
    it('creates non-secure urls if secure is set to false', () => {
      ...
      expect(urlBuilder()({ cloudName: 'demo', secure: false })('test', { secure: true })).toBe(
        `https${insecureUrl}`,
      );
    });
